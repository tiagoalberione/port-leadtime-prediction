...
EXPECTED_REBUILD_CONTEXT = "historical-academic-rebuild"
...
def assert_official_context() -> dict[str, str]:
    """Documenta o contexto Git usado na execucao canonica.

    A primeira reconstrucao oficial foi feita durante a fase academica do
    projeto. Na versao publica, a branch atual e registrada apenas como
    metadado para reproducibilidade, sem impor um nome especifico de branch.
    """

    branch = git_value(["branch", "--show-current"])
    status = git_value(["status", "--short", "--branch"])
    if not DATA_FILE.exists():
        raise RuntimeError(f"Base analitica nao encontrada: {DATA_FILE}")
    context_notes: list[str] = []
    return {
        "root": str(ROOT),
        "branch": branch,
        "rebuild_context": EXPECTED_REBUILD_CONTEXT,
